...
import StateTransfer from "@/components/state-transfer"
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedMeasurements) {
        ...
        const measurementsArray = Array.isArray(parsedMeasurements) ? parsedMeasurements : []
      ...
      if (savedDiaperChanges) {
        ...
        const diaperChangesArray = Array.isArray(parsedDiaperChanges) ? parsedDiaperChanges : []
  ...
  const addMeasurement = (measurement: GrowthMeasurement) => {
    ...
    try {
      localStorage.setItem("growthMeasurements", JSON.stringify(updatedMeasurements))
    } catch (error) {
  ...
  const deleteMeasurement = (measurementId: string) => {
    const updatedMeasurements = measurements.filter((measurement) => measurement.id !== measurementId)
    setMeasurements(updatedMeasurements)
    try {
      localStorage.setItem("growthMeasurements", JSON.stringify(updatedMeasurements))
    } catch (error) {
  ...
  const updateDiaperChange = (updatedChange: DiaperChange) => {
    const updatedChanges = diaperChanges.map((change) => (change.id === updatedChange.id ? updatedChange : change))
    setDiaperChanges(updatedChanges)
    try {
      localStorage.setItem("diaperChanges", JSON.stringify(updatedChanges))
    } catch (error) {
  ...
  const deleteDiaperChange = (changeId: string) => {
    const updatedChanges = diaperChanges.filter((change) => change.id !== changeId)
    setDiaperChanges(updatedChanges)
    try {
      localStorage.setItem("diaperChanges", JSON.stringify(updatedChanges))
    } catch (error) {
  ...
  const importData = (
    ...
    const sessionsArray = Array.isArray(importedSessions) ? importedSessions : []
    const eventsArray = Array.isArray(importedEvents) ? importedEvents : []
    const measurementsArray = Array.isArray(importedMeasurements) ? importedMeasurements : []
    const diaperChangesArray = Array.isArray(importedDiaperChanges) ? importedDiaperChanges : []
    ...
    setMeasurements(measurementsArray)
    try {
      localStorage.setItem("growthMeasurements", JSON.stringify(measurementsArray))
    } catch (error) {
    ...
    <main className="flex min-h-screen flex-col items-center p-4 max-w-md mx-auto">
      ...
      <Tabs defaultValue="feeding" className="w-full">
        ...
        <TabsContent value="feeding" className="w-full">
          <BreastfeedingTracker onSessionComplete={saveSession} nextBreast={nextBreast} />
          ...
          <div className="w-full mt-8">
            ...
            <HistoryList sessions={sessions} onSessionUpdate={updateSession} onSessionDelete={deleteSession} />
          </div>
        </TabsContent>
        ...
        <TabsContent value="events" className="w-full">
          <EventsView events={events} onEventAdd={addEvent} onEventUpdate={updateEvent} onEventDelete={deleteEvent} />
        </TabsContent>
      ...
      <div className="mt-8 flex justify-center gap-2">
        ...
        <GenerateFakeData
          onGenerate={(data) => {
            console.log("Fake data generated, calling importData")
            importData(data.sessions, data.events, data.measurements, data.diaperChanges)
          }}
        />
      </div>
    </main>
