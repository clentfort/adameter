...
- **Key Files**:
  ...
  - `src/i18n/translations.json`: The compiled, runtime translations used by the app.
...
Before submitting your changes, please perform the following steps on **all files you have modified** for the current task:

1.  **Format Touched Files with Prettier**:
    Ensure all files you've changed are formatted correctly. You can typically achieve this by running Prettier directly on the specific files. For example:

    ```bash
    pnpm prettier --write path/to/your/modified/file1.ts path/to/your/modified/file2.tsx
    ```

    _(Consult Prettier documentation for precise commands if needed, or integrate it with your development environment for auto-formatting on save.)_

2.  **Lint and Fix Touched Files with ESLint**:
    Run ESLint with the `--fix` flag to automatically correct linting issues in the files you've edited.
...
5.  **Check for i18n String Changes (if you added/modified user-facing text)**:
    ```bash
    pnpm run fbtee:manifest
    pnpm run fbtee:collect
