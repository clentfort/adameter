...
export async function encryptMergeableContent(
	content: MergeableContent,
	key: CryptoKey,
): Promise<string> {
	return encryptValue(jsonStringWithUndefined(content), key);
}

export async function decryptMergeableContent(
	encryptedContent: string,
	key: CryptoKey,
): Promise<MergeableContent> {
	const decrypted = await decryptValue(encryptedContent, key);
	if (typeof decrypted !== 'string') {
		throw new Error('Decrypted mergeable content is not a string');
	}
	return jsonParseWithUndefined<MergeableContent>(decrypted);
}
