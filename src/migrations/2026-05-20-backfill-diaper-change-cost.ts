...
export const backfillDiaperChangeCostMigration: Migration = {
	description: 'Backfill cost field for existing diaper changes based on product cost',
	...
	migrate: (store) => {
		...
		for (const [changeId, change] of Object.entries(changes)) {
			...
			if (productId && products[productId]) {
				const costPerDiaper = products[productId].costPerDiaper as number | undefined;
				if (typeof costPerDiaper === 'number' && change.cost === undefined) {
					store.setCell(TABLE_IDS.DIAPER_CHANGES, changeId, 'cost', costPerDiaper);
					hasChanged = true;
				}
			}
		}
