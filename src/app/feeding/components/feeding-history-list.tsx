...
export default function HistoryList() {
	...
	const {
		historyRowIds: rowIds,
		remove: onSessionDelete,
		update: onSessionUpdate,
	} = useFeedingSessions();

	const sessionsStub = useMemo(
		() => rowIds.map((id) => ({ id })) as unknown as FeedingSession[],
		[rowIds],
	);
	...
	return (
		<>
			<HistoryListInternal
				dateAccessor={(session) => session.id}
				entries={sessionsStub}
			>
				{(session) => (
					<FeedingHistoryEntry
...
interface FeedingHistoryEntryProps {
	onDelete: (id: string) => void;
	onEdit: (session: FeedingSession) => void;
	sessionId: string;
}
...
function FeedingHistoryEntry({
	...
	onEdit,
	sessionId,
}: FeedingHistoryEntryProps) {
	const session = useFeedingSessionRow(sessionId);
			...
			<div className="flex justify-between items-start">
				...
				<div className="text-right flex flex-col items-end">
					...
					<div className="flex gap-1 mt-2">
						<EditIconButton onClick={() => onEdit(session)} />
