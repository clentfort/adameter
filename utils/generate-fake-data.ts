...
const generateDescription = (type: "event" | "measurement" | "diaper"): string => {
...
Nächste Größe ausprobieren?`,
  ...
  const descriptions =
    type === "event" ? eventDescriptions : type === "measurement" ? measurementDescriptions : diaperDescriptions
...
export const generateFakeData = (
  ...
  console.log(`Generating fake data for ${days} days with ${feedingsPerDay} feedings per day`)
  ...
  const illnessEndDate = hasEndDate
    ? new Date(illnessStartDate.getTime() + randomInt(2, 5) * 24 * 60 * 60 * 1000)
    : undefined
  ...
  for (let i = 0; i < randomMeasurementCount; i++) {
    ...
    const daysSinceBirth = (measurementDate.getTime() - birthDate.getTime()) / (24 * 60 * 60 * 1000)
    ...
    measurements.push({
      ...
      date: measurementDate.toISOString(),
      weight: measurementType < 0.7 ? Math.round(estimatedWeight + weightVariation) : undefined,
      height: measurementType > 0.3 ? Math.round((estimatedHeight + heightVariation) * 10) / 10 : undefined,
      notes: Math.random() > 0.5 ? generateDescription("measurement") : undefined,
    })
  }
  ...
  while (currentDate < endDate && diaperCount < totalDiapers) {
    ...
    const dailyDiapers = randomInt(Math.max(1, diapersPerDay - 2), diapersPerDay + 2) // Slight variation in daily diapers
    ...
    for (let i = 0; i < dailyDiapers && diaperCount < totalDiapers; i++) {
      ...
      const diaperBrand = hasBrand ? diaperBrands[randomInt(0, diaperBrands.length - 1)] : undefined
      ...
      const abnormalities = hasAbnormalities ? generateDescription("diaper") : undefined
  ...
  while (currentDate < endDate && feedingCount < totalFeedings) {
    ...
    const dailyFeedings = randomInt(Math.max(1, feedingsPerDay - 2), feedingsPerDay + 2) // Slight variation in daily feedings
    ...
    for (let i = 0; i < dailyFeedings && feedingCount < totalFeedings; i++) {
      ...
      lastBreast = Math.random() < 0.8 ? (lastBreast === "left" ? "right" : "left") : lastBreast
      ...
      if (r < 0.7) {
        ...
        durationInMinutes = Math.random() < 0.5 ? randomInt(5, 10) : randomInt(35, 65)
      }
  ...
  diaperChanges.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
