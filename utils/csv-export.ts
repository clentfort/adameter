...
export const feedingSessionsToCsv = (sessions: FeedingSession[]): string => {
  ...
  // Create header row
  const header = "Brust,Startzeit,Endzeit,Dauer (Sekunden),Dauer (formatiert)\n"

  // Format each session as a CSV row
  const rows = sessions
    .map((session) => {
      const startTime = format(new Date(session.startTime), "dd.MM.yyyy HH:mm:ss", { locale: de })
      const endTime = format(new Date(session.endTime), "dd.MM.yyyy HH:mm:ss", { locale: de })
...
export const eventsToCsv = (events: Event[]): string => {
  ...
  const rows = events
    .map((event) => {
      const startTime = format(new Date(event.startDate), "dd.MM.yyyy HH:mm:ss", { locale: de })
      const endTime = event.endDate ? format(new Date(event.endDate), "dd.MM.yyyy HH:mm:ss", { locale: de }) : ""
...
export const growthMeasurementsToCsv = (measurements: GrowthMeasurement[]): string => {
  ...
  const rows = measurements
    .map((measurement) => {
      const date = format(new Date(measurement.date), "dd.MM.yyyy", { locale: de })
      const weight = measurement.weight !== undefined ? measurement.weight.toString() : ""
      const height = measurement.height !== undefined ? measurement.height.toString() : ""
...
export const diaperChangesToCsv = (changes: DiaperChange[]): string => {
  ...
  // Create header row
  const header = "Zeitpunkt,Typ,Temperatur,Windelmarke,Ausgelaufen,Auffälligkeiten\n"

  // Format each change as a CSV row
  const rows = changes
    .map((change) => {
      const timestamp = format(new Date(change.timestamp), "dd.MM.yyyy HH:mm:ss", { locale: de })
      const type = change.containsStool ? "Stuhl" : "Nur Urin"
      const temperature = change.temperature !== undefined ? change.temperature.toString() : ""
...
export const downloadAllAsZip = async (
  ...
  try {
    ...
    zip.file("alle_daten.json", createJsonExport(sessions, events, measurements, diaperChanges))
