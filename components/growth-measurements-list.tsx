...
export default function GrowthMeasurementsList({
  ...
  const [measurementToEdit, setMeasurementToEdit] = useState<GrowthMeasurement | null>(null)
  ...
  if (measurementsArray.length === 0) {
    return <p className="text-muted-foreground text-center py-4">{t("noMeasurementsRecorded")}</p>
  }
    ...
    <>
      <div className="space-y-4">
        {sortedMeasurements.map((measurement) => {
          ...
          return (
            <div key={measurement.id} className="border rounded-lg p-4 shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <p className="font-medium text-lg">{format(measurementDate, "dd. MMMM yyyy", { locale: de })}</p>
                  <div className="mt-2 space-y-1">
                    {measurement.weight && (
                      <p className="text-sm">
                        <span className="font-medium">{`${t("weight").split(" ")[0]}:`}</span> {measurement.weight} g
                      </p>
                    )}
                    {measurement.height && (
                      <p className="text-sm">
                        <span className="font-medium">{`${t("height").split(" ")[0]}:`}</span> {measurement.height} cm
                      </p>
                    )}
                    {measurement.notes && <p className="text-sm text-muted-foreground mt-2">{measurement.notes}</p>}
                  </div>
                </div>
      ...
      <AlertDialog open={!!measurementToDelete} onOpenChange={(open) => !open && setMeasurementToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("deleteMeasurement")}</AlertDialogTitle>
            <AlertDialogDescription>{t("deleteMeasurementConfirmation")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
