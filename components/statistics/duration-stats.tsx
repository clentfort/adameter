...
export default function DurationStats({ sessions = [] }: DurationStatsProps) {
  ...
  return (
    <StatsCard title={t("averageFeedingDuration")}>
      <div className="text-2xl font-bold">{formatDuration(avgDuration.total)}</div>
      <div className="text-xs text-muted-foreground mt-1">
        {t("leftBreast")}: <span className="text-left-breast-dark">{formatDuration(avgDuration.left)}</span>
      </div>
      <div className="text-xs text-muted-foreground">
        {t("rightBreast")}: <span className="text-right-breast-dark">{formatDuration(avgDuration.right)}</span>
      </div>
    </StatsCard>
  )
}
