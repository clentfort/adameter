...
export default function TotalFeedingsStats({ sessions = [] }: TotalFeedingsStatsProps) {
  ...
  return (
    <StatsCard title={t("totalFeedings")}>
      ...
      <div className="text-xs text-muted-foreground mt-1">
        {t("leftBreast")}: <span className="text-left-breast-dark">{leftCount}</span>
      </div>
      <div className="text-xs text-muted-foreground">
        {t("rightBreast")}: <span className="text-right-breast-dark">{rightCount}</span>
      </div>
    </StatsCard>
  )
}
