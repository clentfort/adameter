...
export default function StatisticsView({
  ...
  const filteredSessions = (() => {
    ...
    return sessionsArray.filter((session) => new Date(session.startTime) >= cutoffDate)
  })()
  ...
  const filteredDiaperChanges = (() => {
    ...
    const daysToLookBack = Number.parseInt(timeRange)
    const cutoffDate = addDays(now, -daysToLookBack)
    return diaperChangesArray.filter((change) => new Date(change.timestamp) >= cutoffDate)
  })()
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Select value={timeRange} onValueChange={(value) => setTimeRange(value as "7" | "14" | "30" | "all")}>
      ...
      {filteredSessions.length === 0 && filteredDiaperChanges.length === 0 && measurementsArray.length === 0 ? (
        ...
        <>
          ...
          {filteredSessions.length > 0 ? (
            ...
            <div className="text-center py-4 text-muted-foreground">{t("noFeedingDataAvailable")}</div>
          )}
          ...
          {filteredDiaperChanges.length > 0 ? (
            ...
            <div className="text-center py-4 text-muted-foreground">{t("noDiaperDataAvailable")}</div>
          )}

          <h3 className="text-lg font-medium mt-8 mb-4">{t("growthTab")}</h3>
          {measurementsArray.length > 0 ? (
            <GrowthChart measurements={measurementsArray} events={eventsArray} />
          ) : (
            <div className="text-center py-4 text-muted-foreground">{t("noGrowthDataAvailable")}</div>
          )}
        </>
      )}
    </div>
